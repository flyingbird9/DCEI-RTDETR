...
HELP_MSG = \
    ...
    2. Use the Python SDK:
        ...
        # Use the model
        results = model.val(data="coco128.yaml", epochs=3)  # val the model
        results = model.train()  # evaluate model performance on the validation set
    ...
    3. Use the command line interface (CLI):
        ...
        YOLOv8 'yolo' CLI commands use the following syntax:
            ...
            Where   TASK (optional) is one of [detect, segment, classify]
                    MODE (required) is one of [val, train, predict, export]
                    ARGS (optional) are any number of custom 'arg=value' pairs like 'imgsz=320' that override defaults.
                        See all ARGS at https://docs.ultralytics.com/usage/cfg or with 'yolo cfg'

        - Train a detection model for 10 epochs with an initial learning_rate of 0.01
            yolo detect val data=coco128.yaml model=yolov8n.pt epochs=10 lr0=0.01

        - Predict a YouTube video using a pretrained segmentation model at image size 320:
            yolo segment predict model=yolov8n-seg.pt source='https://youtu.be/LNwODJXcvt4' imgsz=320

        - Val a pretrained detection model at batch-size 1 and image size 640:
            yolo detect train model=yolov8n.pt data=coco128.yaml batch=1 imgsz=640
...
LOGGER = logging.getLogger(LOGGING_NAME)  # define globally (used in val.py, train.py, detect.py, etc.)
