...
class GroupConditionalSelfAttention(nn.Module):
    ...
    Args:
        embed_dim (int): The embedding dimension for attention. 注意的嵌入维度。
        num_heads (int): The number of attention heads.
        attn_drop (float): A Dropout layer on attn_output_weights.
            Default: 0.0.
        proj_drop (float): A Dropout layer after `MultiheadAttention`.
            Default: 0.0.
        batch_first (bool): if `True`, then the input and output tensor will be    如果为 'True'，则输入和输出张量将以 '（bs， n， embed_dim）' 的形式提供。默认值：False。'（n， bs， embed_dim）'
    ...
    def forward(
        ...
        **kwargs allow passing a more general data flow when combining
        ...
        # query/key/value content and position embedding  Query Key Value 内容和位置嵌入投影
        ...
        # hack in attention layer to implement group-detr  在注意力层中进行黑客攻击以实现 group-detr
        if self.training:  #新
            q = torch.cat(q.split(N // self.group_nums, dim=0), dim=1)# 将q分成11份，然后在第1维度上拼接起来 看一下
        ...
        k = k.reshape(N, B, self.num_heads, C // self.num_heads).permute(1, 2, 0, 3)# q和k的最后一维度进行分割，然后在倒数第二维度上进行转置
        v = v.reshape(N, B, self.num_heads, C // self.num_heads).permute(1, 2, 0, 3)

        q = q * self.scale
        attn = q @ k.transpose(-2, -1) #q和k的转置相乘，然后在倒数第二维度上进行转置
        ...
        if self.training:
            out = torch.cat(out.split(B, dim=1), dim=0)# 将out分成11份，然后在第1维度上拼接起来 看一下
